# Metabolomics Analysis - Venn Diagrams

source("src/load_packages.R")

df.interact <-
  read_excel("files/caltech.PD mice.011920.xlsx", sheet = "interaction.regcoef") %>% 
  janitor::clean_names() 
...
venn.diagram(
  x = list(
    df.interact.A %>% filter(tissue == "Brainstem", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Cortex", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Nigra", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Striatum", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist()
  ),
  category.names = c("Brainstem", "Cortex" , "Nigra", "Striatum"),
...
venn.diagram(
  x = list(
    df.interact.A %>% filter(tissue == "Brainstem", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Cortex", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Nigra", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Striatum", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist()
  ),
  category.names = c("Brainstem", "Cortex" , "Nigra", "Striatum"),
...
venn.diagram(
  x = list(
    df.interact.A %>% filter(tissue == "Cecum", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Colon", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Colon Content", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Duodenum", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Duodenum Content", pr_t < 0.05, variable == "GenotypeWT") %>% select(metabolite_name) %>% unlist()
  ),
  category.names = c("Cecum", "Colon", "Colon Content", "Duodenum", "Duodenum Content"),
...
venn.diagram(
  x = list(
    df.interact.A %>% filter(tissue == "Cecum", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Colon", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Colon Content", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Duodenum", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist(),
    df.interact.A %>% filter(tissue == "Duodenum Content", pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% select(metabolite_name) %>% unlist()
  ),
  category.names = c("Cecum", "Colon", "Colon Content", "Duodenum", "Duodenum Content"),
...
df.sig.G <- 
  df.interact.A %>% filter(pr_t < 0.05, variable == "GenotypeWT") %>% 
  select(metabolite, tissue) %>% 
  pivot_wider(names_from = "tissue", 
              values_from = "tissue", 
              values_fill = "0",
...
df.sig.interact <- 
  df.interact.A %>% filter(pr_t < 0.05, variable == "MicrobiotaSPF:GenotypeWT") %>% 
  select(metabolite, tissue) %>% 
  pivot_wider(names_from = "tissue", 
              values_from = "tissue", 
              values_fill = "0",
